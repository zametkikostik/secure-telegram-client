...
pub fn find_asset(release: &Release, target: &str) -> Result<Asset> {
    ...
    #[cfg(target_os = "windows")]
    {
        if target.contains("windows") {
            if let Some(a) = release.assets.iter().find(|a| {
                a.name.contains("windows") || a.name.ends_with(".exe")
            }).cloned() {
                return Ok(a);
            }
        }
    ...
    #[cfg(target_os = "macos")]
    {
        if target.contains("darwin") || target.contains("apple") {
            if let Some(a) = release.assets.iter().find(|a| {
                a.name.contains("macos") || a.name.contains("darwin") || a.name.contains("apple")
            }).cloned() {
                return Ok(a);
            }
        }
    ...
    Err(anyhow!("Не найден подходящий ассет для платформы: {}", target))
}
