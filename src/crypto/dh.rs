...
impl X25519KeyPair {
    ...
    pub fn compute_shared_secret(&self, peer_public_key: &[u8; X25519_PUBLIC_KEY_SIZE]) -> [u8; X25519_SHARED_SECRET_SIZE] {
...
impl KeyExchange {
    ...
    pub fn derive_key(shared_secret: &[u8; X25519_SHARED_SECRET_SIZE], salt: &[u8], info: &[u8]) -> [u8; 32] {
        use hkdf::Hkdf;
        use sha2::Sha256;
        
        let hkdf = Hkdf::<Sha256>::new(Some(salt), shared_secret);
        let mut okm = [0u8; 32];
        hkdf.expand(info, &mut okm).expect("32 байт - валидный размер для вывода");
