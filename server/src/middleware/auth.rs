...
pub enum AuthError {
    ...
    #[error("Token invalid: {0}")] TokenInvalid(String),
    #[error("User not found")] UserNotFound,
    #[error("Username taken")] UsernameTaken,
    #[error("DB: {0}")] Database(String),
    #[error("No auth")] MissingAuth,
...
pub struct Claims { pub sub: String, pub username: String, pub exp: usize, pub iat: usize }

#[derive(Clone)]
pub struct AuthState { pub jwt_secret: String, pub jwt_expiry: usize }

impl AuthState {
    pub fn new(s: &str, e: usize) -> Self { Self { jwt_secret: s.into(), jwt_expiry: e } }

    pub fn generate_token(&self, uid: &str, uname: &str) -> Result<String, AuthError> {
        let now = chrono::Utc::now().timestamp() as usize;
        jsonwebtoken::encode(
            &jsonwebtoken::Header::default(),
            &Claims { sub: uid.into(), username: uname.into(), exp: now + self.jwt_expiry, iat: now },
    ...
    pub fn validate_token(&self, t: &str) -> Result<Claims, AuthError> {
        decode::<Claims>(t, &DecodingKey::from_secret(self.jwt_secret.as_bytes()), &Validation::default())
            .map(|d| d.claims)
...
pub fn get_user_id_from_header(auth_header: Option<String>, auth: &AuthState) -> Result<String, AuthError> {
    let auth_str = auth_header.ok_or(AuthError::MissingAuth)?;
    let token = auth_str.strip_prefix("Bearer ").ok_or(AuthError::MissingAuth)?;
    let claims = auth.validate_token(token)?;
    Ok(claims.sub)
}

#[derive(Deserialize)]
pub struct RegisterRequest { pub username: String, pub password: String, pub display_name: Option<String>, pub public_key_x25519: Option<String>, pub public_key_ed25519: Option<String>, pub avatar_url: Option<String> }

#[derive(Deserialize)]
pub struct LoginRequest { pub username: String, pub password: String }

#[derive(Serialize)]
pub struct AuthResponse { pub token: String, pub user_id: String, pub username: String, pub display_name: Option<String> }
