...
use axum::{
    ...
    Json, response::Response, body::Body,
};
...
pub async fn create_story(
    ...
    // Get user info
    let user_row: Option<(String, Option<String>)> = sqlx::query_as(
    ...
    if !stories_dir.exists() {
        fs::create_dir_all(&stories_dir).await.map_err(|e| AuthError::Database(e.to_string()))?;
    }
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| AuthError::Database(e.to_string()))? {
        ...
        if name == "caption" {
            if let Ok(text) = field.text().await { caption = Some(text); }
        } else if name == "media_type" {
            if let Ok(mt) = field.text().await { media_type = mt; }
        } else if name == "media" {
...
pub async fn get_stories(State(s): State<AppState>, headers: HeaderMap) -> Result<Json<Vec<Story>>, AuthError> {
    let auth_hdr = headers.get("authorization").and_then(|v| v.to_str().ok()).map(String::from);
    let uid = get_user_id_from_header(auth_hdr, &s.auth)?;
    ...
    let result: Vec<Story> = stories.into_iter().map(|(id, user_id, username, display_name, _avatar_url, media_url, media_type, caption, created_at, expires_at)| {
        Story { id, user_id, username, display_name: Some(display_name), avatar_url: None, media_url, media_type, caption: if caption.is_empty() { None } else { Some(caption) }, created_at, expires_at, view_count: 0, is_viewed: false }
    }).collect();
...
pub async fn get_user_stories(State(s): State<AppState>, Path(user_id): Path<String>) -> Result<Json<Vec<Story>>, (StatusCode, Json<serde_json::Value>)> {
    let now = Utc::now().to_rfc3339();
    let stories: Vec<(String, String, String, String, Option<String>, String, String, String, String, String)> = sqlx::query_as(
    ...
    let result: Vec<Story> = stories.into_iter().map(|(id, uid, username, display_name, _avatar_url, media_url, media_type, caption, created_at, expires_at)| {
        Story { id, user_id: uid, username, display_name: Some(display_name), avatar_url: None, media_url, media_type, caption: if caption.is_empty() { None } else { Some(caption) }, created_at, expires_at, view_count: 0, is_viewed: false }
    }).collect();
...
/// Get story media
pub async fn get_story_media(State(_s): State<AppState>, Path(storage_name): Path<String>) -> Result<Response, (StatusCode, Json<serde_json::Value>)> {
    let file_path = PathBuf::from(STORIES_DIR).join(&storage_name);
    if !file_path.exists() {
        return Err((StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Media not found"}))));
    }

    let data = fs::read(&file_path).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
...
pub async fn view_story(State(s): State<AppState>, headers: HeaderMap, Path(story_id): Path<String>) -> Result<Json<serde_json::Value>, AuthError> {
    let auth_hdr = headers.get("authorization").and_then(|v| v.to_str().ok()).map(String::from);
    let uid = get_user_id_from_header(auth_hdr, &s.auth)?;

    // Record view (ignore duplicates)
    let _ = sqlx::query("INSERT OR IGNORE INTO story_views(story_id, user_id, viewed_at) VALUES(?,?,?)")
        .bind(&story_id).bind(&uid).bind(&Utc::now().to_rfc3339())
        .execute(&*s.db).await;

    // Increment counter
    let _ = sqlx::query("UPDATE stories SET view_count = view_count + 1 WHERE id=?").bind(&story_id).execute(&*s.db).await;

    Ok(Json(serde_json::json!({"viewed": true})))
}

/// Delete story (owner only)
pub async fn delete_story(State(s): State<AppState>, headers: HeaderMap, Path(story_id): Path<String>) -> Result<Json<serde_json::Value>, AuthError> {
    let auth_hdr = headers.get("authorization").and_then(|v| v.to_str().ok()).map(String::from);
    let uid = get_user_id_from_header(auth_hdr, &s.auth)?;

    // Verify ownership
    let owner: Option<String> = sqlx::query_scalar("SELECT user_id FROM stories WHERE id=?").bind(&story_id).fetch_optional(&*s.db).await.map_err(|e| AuthError::Database(e.to_string()))?;
    if owner.as_deref() != Some(&uid) {
        return Err(AuthError::UserNotFound);
    }

    sqlx::query("DELETE FROM stories WHERE id=?").bind(&story_id).execute(&*s.db).await.map_err(|e| AuthError::Database(e.to_string()))?;
    sqlx::query("DELETE FROM story_views WHERE story_id=?").bind(&story_id).execute(&*s.db).await.ok();
