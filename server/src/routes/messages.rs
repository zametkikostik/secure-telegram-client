use axum::{extract::{Path,Query,State},http::{StatusCode,HeaderMap},Json};
...
pub async fn send_message(
    ...
    let p:Option<i64>=sqlx::query_scalar("SELECT COUNT(*) FROM chat_participants WHERE chat_id=? AND user_id=?")
        .bind(&cid).bind(&uid).fetch_optional(&*s.db).await.map_err(|e|AuthError::Database(e.to_string()))?;
    if p.unwrap_or(0)==0 { return Err(AuthError::UserNotFound); }
    ...
    let destroy_at = r.destroy_after_seconds.map(|secs| {
    ...
    let sender_name:Option<String>=sqlx::query_scalar("SELECT COALESCE(display_name, username) FROM users WHERE id=?")
    ...
    let participants:Vec<String>=sqlx::query_scalar("SELECT user_id FROM chat_participants WHERE chat_id=?")
        .bind(&cid).fetch_all(&*s.db).await.unwrap_or_default();

    for participant_id in participants {
        if participant_id != uid { // Don't send to sender
            s.ws_manager.send_to_user(&participant_id, ws_msg.clone()).await;
        }
    }

    Ok((StatusCode::CREATED, Json(Message {
        id:mid, chat_id:cid, sender_id:uid, sender_name:None,
        content:r.content, msg_type:mt, created_at:now, edited_at:None,
        reply_to:r.reply_to, is_encrypted:false, ephemeral_key:None, nonce:None,
    })))
}
...
pub async fn send_message_e2ee(
    ...
    Ok((StatusCode::CREATED, Json(Message {
        id:mid, chat_id:cid, sender_id:uid, sender_name:None,
        content:r.payload.ciphertext, msg_type:"e2ee".into(), created_at:now, edited_at:None,
        reply_to:r.reply_to, is_encrypted:true,
        ephemeral_key:Some(r.payload.ephemeral_public_key), nonce:Some(r.payload.nonce),
    })))
}
...
#[derive(Debug,Serialize,Deserialize,FromRow)]
pub struct ReadReceipt { pub message_id:String, pub user_id:String, pub username:Option<String>, pub read_at:String }
...
pub async fn mark_read(
    ...
    sqlx::query("INSERT OR IGNORE INTO read_receipts(message_id,user_id,read_at)VALUES(?,?,?)")
        .bind(&mid).bind(&uid).bind(&now).execute(&*s.db).await.map_err(|e|AuthError::Database(e.to_string()))?;
    Ok(Json(()))
}
...
pub async fn get_unread_count(
    ...
    Ok(Json(serde_json::json!({ "chat_id": cid, "unread_count": count.unwrap_or(0) })))
}
...
#[derive(Deserialize)]
pub struct ListMessagesParams { pub limit:Option<usize>, pub offset:Option<usize> }

#[derive(Deserialize)]
pub struct AddReactionRequest { pub emoji: String }
...
pub async fn add_reaction(
    ...
    sqlx::query("INSERT OR REPLACE INTO reactions(message_id,user_id,emoji,created_at)VALUES(?,?,?,?)")
        .bind(&mid).bind(&uid).bind(&r.emoji).bind(&now)
        .execute(&*s.db).await.map_err(|e|AuthError::Database(e.to_string()))?;

    let count:Option<i64>=sqlx::query_scalar("SELECT COUNT(*) FROM reactions WHERE message_id=? AND emoji=?")
        .bind(&mid).bind(&r.emoji).fetch_optional(&*s.db).await.map_err(|e|AuthError::Database(e.to_string()))?;
...
pub async fn get_message_reactions(
    ...
    let mut grouped:std::collections::HashMap<String,(i64,Vec<String>)>=std::collections::HashMap::new();
    ...
    let reactions_vec:Vec<_>=grouped.into_iter().map(|(emoji,(count,users))|serde_json::json!({
        "emoji": emoji,
        "count": count,
        "users": users,
    })).collect();
...
pub async fn typing_indicator(
    ...
    let username:Option<String>=sqlx::query_scalar("SELECT COALESCE(display_name,username) FROM users WHERE id=?")
...
pub async fn edit_message(
    ...
    let sender_id: Option<String> = sqlx::query_scalar("SELECT sender_id FROM messages WHERE id = ?")
        .bind(&mid)
        .fetch_optional(&*s.db)
        .await
    ...
    match sender_id {
        Some(sid) if sid == uid => {
            ...
            Ok((StatusCode::OK, Json(serde_json::json!({ "edited": true, "edited_at": now }))))
        }
...
#[derive(Deserialize)]
pub struct TypingRequest { pub is_typing: bool }
...
pub async fn pin_message(
    ...
    Ok((StatusCode::OK, Json(serde_json::json!({ "success": true, "chat_id": chat_id }))))
}
...
pub async fn unpin_message(
    ...
    sqlx::query("DELETE FROM pinned_messages WHERE message_id = ?").bind(&mid)
        .execute(&*s.db).await.map_err(|e| AuthError::Database(e.to_string()))?;
