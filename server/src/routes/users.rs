use crate::middleware::auth::{
    get_user_id_from_header, AuthError, AuthResponse, LoginRequest, RegisterRequest,
};
use crate::AppState;
use axum::{
    extract::{Path, State},
...
pub async fn register(
    ...
    if r.username.len() < 3 || r.username.len() > 50 || r.password.len() < 6 {
        return Err(AuthError::InvalidCredentials);
    }
    let ex: Option<i64> = sqlx::query_scalar("SELECT COUNT(*)FROM users WHERE username=?")
        .bind(&r.username)
        .fetch_one(&*s.db)
        .await
...
pub async fn login(
    ...
    let (uid, ph, dn, pk_x, pk_e) = match row {
        Some(r) => r,
        None => return Err(AuthError::InvalidCredentials),
    };
    if !s.auth.verify_password(&r.password, &ph)? {
        return Err(AuthError::InvalidCredentials);
    }
    let token = s.auth.generate_token(&uid, &r.username)?;
    // Set user online on login
    let now = chrono::Utc::now().to_rfc3339();
    sqlx::query("UPDATE users SET is_online=1, last_seen=? WHERE id=?")
        .bind(&now)
        .bind(&uid)
        .execute(&*s.db)
        .await
...
pub async fn update_keys(
    ...
    sqlx::query("UPDATE users SET public_key_x25519=?, public_key_ed25519=? WHERE id=?")
        .bind(&r.public_key_x25519)
        .bind(&r.public_key_ed25519)
        .bind(&uid)
        .execute(&*s.db)
        .await
        .map_err(|e| AuthError::Database(e.to_string()))?;
...
pub async fn get_keys(
    State(s): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<crate::e2ee::PublicKeyBundle>, AuthError> {
    let row: Option<(Option<String>, Option<String>)> =
        sqlx::query_as("SELECT public_key_x25519, public_key_ed25519 FROM users WHERE id=?")
            .bind(&id)
            .fetch_optional(&*s.db)
            .await
            .map_err(|e| AuthError::Database(e.to_string()))?;
...
pub async fn get_user_status(
    ...
    let row: Option<(i64, Option<String>)> =
        sqlx::query_as("SELECT is_online, last_seen FROM users WHERE id=?")
            .bind(&id)
            .fetch_optional(&*s.db)
            .await
            .map_err(|e| AuthError::Database(e.to_string()))?;
    ...
    Ok(Json(
        serde_json::json!({ "user_id": id, "is_online": online, "last_seen": last_seen }),
    ))
}
