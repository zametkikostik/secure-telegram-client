...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxExecuteRequest {
    pub tx_id: String,
    pub tx_hash: String,
}

/// Ответ выполнения транзакции
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxExecuteResponse {
    pub success: bool,
    pub record: Option<TransactionRecord>,
    pub error: Option<String>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxHistoryRequest {
    pub address: Option<String>,
    pub limit: Option<usize>,
    pub offset: Option<usize>,
}

/// Ответ истории транзакций
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Tauri команда: создать транзакцию
#[tauri::command]
pub fn tx_create(
    request: TxCreateRequest,
    state: State<'_, TxTauriState>,
) -> Result<TxCreateResponse, String> {
    ...
    let tx_type = match request.tx_type.as_str() {
        ...
        "approval" => TxType::Approval,
        "tip" => TxType::Tip,
        _ => return Err(format!("Unknown tx_type: {}", request.tx_type)),
    };
    ...
    let id = state.store.create(tx_request);
...
/// Tauri команда: обновить статус транзакции
#[tauri::command]
pub fn tx_update_status(
    tx_id: String,
    status: String,
    ...
    let record = match status.as_str() {
        "submitted" => {
            let hash = tx_hash.ok_or("tx_hash required for submitted status")?;
            state.store.mark_submitted(&tx_id, &hash)
        }
        "confirmed" => {
            let hash = tx_hash.ok_or("tx_hash required for confirmed status")?;
            let block = block_number.ok_or("block_number required for confirmed status")?;
            state.store.mark_confirmed(&tx_id, &hash, block)
        }
        "failed" => {
...
/// Tauri команда: получить историю транзакций
#[tauri::command]
pub fn tx_get_history(
    request: TxHistoryRequest,
    state: State<'_, TxTauriState>,
) -> Result<TxHistoryResponse, String> {
    let all_txs = state.store.list();
...
/// Tauri команда: получить статус транзакции
#[tauri::command]
pub fn tx_get_status(
    request: TxStatusRequest,
    state: State<'_, TxTauriState>,
) -> Result<TxStatusResponse, String> {
    let record = state.store.get(&request.tx_id);

    let success = record.is_some();
    let error_msg = if !success {
        Some(format!("Transaction not found: {}", request.tx_id))
    } else {
        None
    };
...
/// Tauri команда: очистить подтверждённые транзакции
#[tauri::command]
pub fn tx_prune_confirmed(
    keep_last: usize,
    state: State<'_, TxTauriState>,
) -> Result<usize, String> {
    let before = state.store.count();
    state.store.prune_confirmed(keep_last);
    let after = state.store.count();
