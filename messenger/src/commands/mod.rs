...
use crate::ai::{
    AiClient, AiConfig, AiProvider, ModelInfo, ModelHint, ProviderStatus,
};
...
#[tauri::command]
pub async fn get_available_providers(state: tauri::State<'_, AiState>) -> Result<Vec<ProviderInfo>, String> {
...
#[tauri::command]
pub async fn get_available_models(state: tauri::State<'_, AiState>) -> Result<Vec<ModelInfoDto>, String> {
    let client = state.client.lock().unwrap();
...
use crate::monetization::{MonetizationManager, SubscriptionTier, PaymentMethod, PremiumFeature, CreditSource};
...
mod web3_commands {
    ...
    #[tauri::command]
    pub async fn get_wallet_info(state: tauri::State<'_, Web3State>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn get_balance(chain: String, state: tauri::State<'_, Web3State>) -> Result<serde_json::Value, String> {
    let chain_enum: Chain = chain.parse()
        .map_err(|e: String| e)?;
...
pub async fn get_transactions(state: tauri::State<'_, Web3State>) -> Vec<serde_json::Value> {
    ...
    txs.into_iter().map(|tx| serde_json::json!({
        "id": tx.id,
        "chain": tx.chain.name(),
...
pub async fn update_ad_preferences(
    ...
    if let Some(cats) = preferences.get("preferred_categories").and_then(|v| v.as_array()) {
        prefs.preferred_categories = cats.iter()
            .filter_map(|v| v.as_str().and_then(|s| s.parse::<AdCategory>().ok()))
            .collect();
    }

    if let Some(cats) = preferences.get("blocked_categories").and_then(|v| v.as_array()) {
        prefs.blocked_categories = cats.iter()
            .filter_map(|v| v.as_str().and_then(|s| s.parse::<AdCategory>().ok()))
            .collect();
    }
...
pub async fn record_ad_impression(
    ...
    state.ad_engine.record_impression(&ad_id, duration_secs)
...
pub async fn record_ad_click(
    ...
    state.ad_engine.record_click(&ad_id)
...
/// Get credit history
#[tauri::command]
pub async fn get_credit_history(state: tauri::State<'_, Web3State>) -> Result<Vec<serde_json::Value>, String> {
    let address = state.get_user_address()
        .ok_or("Wallet not connected")?;

    let history = state.monetization.get_credit_history(&address);
    Ok(history.into_iter().map(|tx| serde_json::json!({
        "id": tx.id,
        "amount": tx.amount,
...
#[tauri::command]
pub async fn get_subscription(state: tauri::State<'_, Web3State>) -> Result<serde_json::Value, String> {
    let address = state.get_user_address()
        .ok_or("Wallet not connected")?;
...
pub async fn subscribe(
    ...
    if pay_method == PaymentMethod::Credits {
        ...
        if balance < cost {
            return Err(format!("Insufficient credits: need {}, have {}", cost, balance));
        }
    ...
    let sub = state.monetization.create_subscription(
        ...
        tier_enum,
        pay_method,
        None, // tx_hash would come from actual payment
    );
...
pub async fn get_premium_features() -> Vec<serde_json::Value> {
    ...
    features.iter().map(|f| serde_json::json!({
        "name": f.name(),
...
#[tauri::command]
pub async fn get_purchased_features(state: tauri::State<'_, Web3State>) -> Result<Vec<String>, String> {
    let address = state.get_user_address()
        .ok_or("Wallet not connected")?;
