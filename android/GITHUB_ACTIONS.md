# 🤖 GitHub Actions — Автоматическая сборка APK

## Настроенные workflow

### 1. Android APK Build (`android-build.yml`)

**Триггеры**:
- Push в ветку `master` (только изменения в `android/`)
- Pull Request
- Ручной запуск (workflow_dispatch)

**Что делает**:
1. ✅ Устанавливает JDK 17
2. ✅ Настраивает Android SDK
3. ✅ Устанавливает Android платформы и NDK
4. ✅ Собирает Debug APK через Gradle
5. ✅ Загружает APK как артефакт
6. ✅ Создаёт релиз (если есть тег)

**Результат**:
- Debug APK доступен в артефактах (30 дней)
- Release APK (при пуше в master)

---

## 📥 Как скачать собранный APK

### Через GitHub Actions

1. Перейдите на вкладку **Actions**: https://github.com/zametkikostik/secure-telegram-client/actions
2. Выберите последний успешный запуск
3. В разделе **Artifacts** скачайте `secure-messenger-debug`
4. Распакуйте ZIP — внутри будет `app-fdroid-debug.apk`

### Через Releases

1. Перейдите на вкладку **Releases**: https://github.com/zametkikostik/secure-telegram-client/releases
2. Скачайте APK из последнего релиза

---

## 🚀 Ручной запуск сборки

1. Перейдите на вкладку **Actions**
2. Выберите workflow **"Android APK Build"**
3. Нажмите **"Run workflow"**
4. Выберите ветку (обычно `master`)
5. Нажмите **"Run workflow"**
6. Дождитесь завершения (~5-10 минут)
7. Скачайте APK из артефактов

---

## ⚙️ Настройки

### Переменные окружения

```yaml
env:
  ANDROID_VERSION: 34
  BUILD_TOOLS_VERSION: 34.0.0
```

### Артефакты

- **Имя**: `secure-messenger-debug`
- **Путь**: `android/app/build/outputs/apk/fdroid/debug/app-fdroid-debug.apk`
- **Срок хранения**: 30 дней

---

## 📊 Статус сборки

[![Android APK Build](https://github.com/zametkikostik/secure-telegram-client/actions/workflows/android-build.yml/badge.svg)](https://github.com/zametkikostik/secure-telegram-client/actions/workflows/android-build.yml)

---

## 🔧 Troubleshooting

### Сборка не удалась

1. Проверьте логи в Actions
2. Убедитесь что `build.gradle.kts` корректен
3. Проверьте версии зависимостей

### APK не загружается

1. Проверьте что путь указан верно
2. Убедитесь что Gradle сборка успешна
3. Проверьте права доступа

---

## 📝 Автоматические релизы

Для создания релиза с APK:

```bash
# Создайте тег
git tag v0.2.0
git push origin v0.2.0
```

GitHub Actions автоматически:
1. Соберёт APK
2. Создаст релиз на GitHub
3. Прикрепит APK к релизу

---

**Теперь GitHub автоматически собирает APK!** 🎉
